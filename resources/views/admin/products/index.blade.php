...
<div class=" bg-cover overflow-y-scroll px-5">
        ...
        <div class="container max-w-4xl mx-auto pt-10 mb-12 text-center break-normal">
           ...
           <form action="{{ Asset($link) }}" method="GET" id="search_form">
                ...
                <input class="w-full h-12 px-3 border-gray-300 border-2 rounded mt-6 focus:outline-none focus:shadow-outline text-lg px-8 shadow-lg" type="search" name="product_name" placeholder="Search Product">
    ...
    <thead>
        <tr>
            <th class="p-3 font-bold uppercase shadow bg-blue-800 text-white border border-gray-300 hidden lg:table-cell">Product name</th>
            <th class="p-3 font-bold uppercase shadow bg-blue-800 text-white border border-gray-300 hidden lg:table-cell">Price</th>
            <th class="p-3 font-bold uppercase shadow bg-blue-800 text-white border border-gray-300 hidden lg:table-cell">Slug</th>
            <th class="p-3 font-bold uppercase shadow bg-blue-800 text-white border border-gray-300 hidden lg:table-cell">Category</th>
    ...
    <tbody>
        ...
        <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0 items-center justify-center">
            ...
            <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
               ...
               <br> {{number_format($product->price)}}
            </td>
            <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
            ...
            <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
                ...
                <br>
                <a href="{{ Asset($link.$product->id.'/edit') }}" class="text-green-500 hover:text-green-700 underline" ><i class="fas fa-edit mr-2 text-green-500"></i>Edit</a>
               
                <a href="{{ Asset($link.$product->id.'/delete') }}" class="text-red-500 hover:text-red-700 underline pl-6" onclick="event.preventDefault();
                						 if(confirm('Delete Product?')){document.getElementById('delete_form_{{$product->id}}').submit()};
                                   "><i class="fas fa-trash mr-2 text-red-500"></i>Delete</a>

                <form id="delete_form_{{$product->id}}" action="{{ Asset($link.$product->id.'/delete') }}" method="POST" style="display: none;">
                  @csrf
                  @method('DELETE')
