...
<head>
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}">
     ...
     <!-- <title>{{ config('app.name', 'AC Enterprise') }}</title> -->
...
<body>
  ...
  <aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl bg-blue-800">
        <div class="p-6">
            <a href="{{ Asset(env('admin'))}}" class="text-white text-3xl font-semibold uppercase hover:text-gray-300">Admin</a>
            <a href="{{ route('seller.create') }}" class="w-full bg-white cta-btn font-semibold py-2 mt-5 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl hover:bg-gray-300 flex items-center justify-center">
        ...
        <nav class="text-white text-base font-semibold pt-1">
            ...
            @if($page == "dashboard")
            <a href="{{ Asset(env('admin').'/dashboard') }}" class="flex items-center active-nav-link text-white py-4 pl-6 nav-item">
            <i class="fas fa-tachometer-alt mr-3"></i>
                Dashboard
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/dashboard') }}">
            <i class="fas fa-tachometer-alt mr-3"></i>
                Dashboard
            </a>
            @endif
            ...
            @if($page == "customers")
            <a class="flex items-center text-white py-4 pl-6 nav-item active-nav-link" href="{{ Asset(env('admin').'/customers') }}">
            <i class="fas fa-users mr-3"></i>
                Customers
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/customers') }}">
            <i class="fas fa-users mr-3"></i>
                Customers
            </a>
            @endif
            ...
            @if($page == "seller")
            <a class="flex items-center text-white py-4 pl-6 nav-item active-nav-link" href="{{ Asset(env('admin').'/seller') }}">
            <i class="fas fa-user-tie mr-3"></i>
                Sellers
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/seller') }}">
            <i class="fas fa-user-tie mr-3"></i>
                Sellers
            </a>
            @endif
            ...
            @if($page == "products")
            <a class="flex items-center text-white py-4 pl-6 nav-item active-nav-link" href="{{ Asset(env('admin').'/products') }}">
            <i class="fas fa-notes-medical mr-3"></i>
                Products
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/products') }}">
            <i class="fas fa-notes-medical mr-3"></i>
                Products
            </a>
            @endif
            ...
            @if($page == "categories")
            <a class="flex items-center text-white py-4 pl-6 nav-item active-nav-link" href="{{ Asset(env('admin').'/categories') }}">
            <i class="fas fa-list mr-3"></i>
                Categories
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/categories') }}">
            <i class="fas fa-list mr-3"></i>
                Categories
            </a>
            @endif
            ...
            @if($page == "orders")
            <a class="flex items-center text-white py-4 pl-6 nav-item active-nav-link" href="{{ Asset(env('admin').'/orders') }}">
            <i class="fas fa-shopping-cart mr-3"></i>
                Orders
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/orders') }}">
            <i class="fas fa-shopping-cart mr-3"></i>
                Orders
            </a>
            @endif
            ...
            @if($page == "sales")
            <a class="flex items-center text-white py-4 pl-6 nav-item active-nav-link" href="{{ Asset(env('admin').'/sales') }}">
            <i class="fas fa-rupee-sign mr-3"></i>
                Sales
            </a>

            @else
            <a class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item" href="{{ Asset(env('admin').'/sales') }}">
            <i class="fas fa-rupee-sign mr-3"></i>
            Sales
            </a>
            @endif
        ...
        @if($page == "settings")
        <a href="{{ Asset(env('admin').'/settings') }}" class="absolute w-full upgrade-btn bottom-0 active-nav-link text-white flex items-center justify-center py-4">
            <i class="fas fa-user mr-3"></i>
            Edit Profile
        </a>
        @else
        <a href="{{ Asset(env('admin').'/settings') }}" class="absolute opacity-75 hover:opacity-100 w-full upgrade-btn bottom-0 text-white flex items-center justify-center py-4">
            <i class="fas fa-user mr-3"></i>
            Edit Profile
        </a>
        @endif
        ...
        <header class="w-full flex items-center bg-gray-500 shadow-2xl py-2 px-6">
            <div class="relative w-full py-4 flex justify-end">
             
                <ul class="inline-flex items-center">

                    <li>
                        <a href="{{ url('/')  }}" target="_blank" class="px-4 hover:text-gray-800"><i class="fas fa-window-restore text-lg px-2"></i>Live Site</a>
                    </li>
                    ...
                    <li>
                        <a href="{{ Asset(env('admin').'/logout') }}" class="px-4 hover:text-gray-800">Logout<i class="fas fa-sign-out-alt text-lg  px-2"></i></a>
     ...
     <!--      @if(Session::has('error'))
                    <div id="card-alert" class="bg-red-500">
                      <div class="text-white text-center py-2">
                        <p><i class="fas fa-exclamation"></i> ERROR : {{ Session::get('error') }}</p>
                      </div>
                    </div>
            @endif
            ...
            @if(Session::has('message'))
                    <div id="card-alert" class="bg-green-500">
                      <div class="text-white text-center py-2">
                        <p><i class="fas fa-check"></i> SUCCESS : {{ Session::get('message') }}</p>
                      </div>
                    </div>
            @endif  -->
